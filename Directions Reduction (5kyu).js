...
function dirReduc(arr) {
  const directionsOptions = ['EASTWEST', 'WESTEAST', 'NORTHSOUTH', 'SOUTHNORTH']

  let findedIndex

  while (findedIndex !== -1) {
    findedIndex = arr.findIndex((element, index) => {
      for (i = 0; i < directionsOptions.length; i++) {
        if (`${arr[index]}${arr[index + 1]}` === directionsOptions[i]) {
          console.log(directionsOptions[i], index)
          return true
        }
      }
    })

    if (findedIndex !== -1) arr.splice(findedIndex, 2)
  }
